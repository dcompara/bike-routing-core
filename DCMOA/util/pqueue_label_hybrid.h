...
// A two-layer bucket-heap queue with Last In First Out (LIFO) strategy
...
class pqueue_label_hybrid
{
	...
	public:
        ...
        pqueue_label_hybrid(uint size=1, cost_t fmin=0, cost_t fmax=0)
        ...
        // {
        //     uint index = (lb->get_f_pri() - f_min)/bucket_width_;
        //     uint index_old = (lb_old->get_f_pri() - f_min)/bucket_width_;
        //     if (index == cursor_)
        //     {
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
// A two-layer cyclic bucket-heap queue with Last In First Out (LIFO) strategy
template <class Heap, class LABEL>
class pqueue_label_hybrid_cyclic
{
	...
	public:
        ...
        pqueue_label_hybrid_cyclic(uint size=1, cost_t fmin=0, cost_t fmax=0)
            : max_cursor_(1), queuesize_(0), cursor_(0), f_min(fmin), cmp(0)
        {
            // bucket_width_ = size;
            if (fmax <= fmin) bucket_width_ = 1;
            else bucket_width_ = (fmax - fmin)/1e6 + 1;  // enforcing 1M bucket max
        ...
        push (LABEL* lb)
        {
            uint index = (lb->get_f_pri() - f_min)/bucket_width_ + offset_;
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
// A two-layer fixed-size cyclic bucket-heap queue with Last In First Out (LIFO) strategy
template <class Heap, class LABEL>
class pqueue_label_hybrid_cyclic_fixed
{
    public:
    
        pqueue_label_hybrid_cyclic_fixed(uint size=1, cost_t fmin=0, cost_t max_delta_f=0)
            : max_cursor_(max_delta_f), queuesize_(0), cursor_(0), f_min(fmin), cmp(0)
        {
            // bucket_width_ = size;
            bucket_width_ = max_delta_f/1e6 + 1;  // enforcing 1M bucket max
            if (max_delta_f > 1e6) max_cursor_ = 1e6;
            offset_ = 0;
            high_level_ = new linkedlist<LABEL*>[max_cursor_ + 1]();
        }

        ~pqueue_label_hybrid_cyclic_fixed()
        {
            delete [] high_level_;
        }
...
typedef pqueue_label_hybrid_cyclic_fixed<pqueue_label_heap_min_w_tie, search_label_pri> pqueue_label_hybrid_cyclic_fixed_min_w_tie;
typedef pqueue_label_hybrid_cyclic_fixed<pqueue_label_heap_min_wo_tie, search_label_pri> pqueue_label_hybrid_cyclic_fixed_min_wo_tie;
typedef pqueue_label_hybrid_cyclic_fixed<pqueue_label_light_heap_min_w_tie, search_label_light_pri> pqueue_label_light_hybrid_cyclic_fixed_min_w_tie;
typedef pqueue_label_hybrid_cyclic_fixed<pqueue_label_light_heap_min_wo_tie, search_label_light_pri> pqueue_label_light_hybrid_cyclic_fixed_min_wo_tie;
#endif