#ifndef NWMOA_3C_H
#define NWMOA_3C_H
// Unidirectional Multi-objective A* Search with Negative Weights
// for the Multi-objective Shortest Path Problem (MOSP) with three objectives and logarithmic (binary) dominance check
// @author: sahmadi
// created: 01/11/2023
// updated: 17/06/2024
...
public:
    NWMOA_3C(graph *G, graph *G_rev, experiment exp) : LABEL_Pool_(1024)
    {
        ...
        ////////////////////////////////////////////////////
        // start the preliminary searches needed to calculate lower bounds
        timer mytimer;
        mytimer.start();
        ...
        {
            // Run Dijkstra's algorithm with re-exansions allowed to calculate lower bounds and detect negative cycles
        ...
        // Priority queue can be a fixed-size cyclic bucket queue
        ...
        // Calculate memory used for all labels + priority queue + all stored paths (expnaded + backtracking)
...
private:
    ...
    sn_id_t *BFS_f;
//////////////////////////////////////////////////////
//// This function initialises the necessary parameters
    void
...
////////////////////////////////////////////////////////
/// The main multi-objective A* serch happens here
void *Multi_search(
        graph *G
        , Q &Open
        , label_pool<LABEL> &label_pool_main
        ...
        , size_t &comp_)
    {
        ...
        LABEL *current_label = label_pool_main.get_label();
        *current_label = LABEL(h[initial], initial, DEG_MAX, PATH_ARR_SIZE_MAX);

        // Insert the initial lablel into the queue
        ...
        while (Open.size())
        {
            ...
            {label_pool_main.save_label(current_label); continue;} // if dominated, thus skip expansion + recycle the label

            // Perform a logarithmic Dominance chcek against previous expansion of the vertex in lexicographical order
            ...
            if (is_dominated) {label_pool_main.save_label(current_label); continue;} // if dominated, thus skip expansion + recycle the label

            // Perform a logarithmic Dominance chcek, this time against the target verex
            ...
            if (is_dominated == true) {label_pool_main.save_label(current_label); continue;}
            ...
            if (current_vertex == target)
            {
                ...
                while(current_sol && current_sol->get_f_pri() == current_label->get_f_pri())
                {
                    ...
                    if (tmp) label_pool_main.save_label(tmp); // recycle the dominated solution label
                }
            ...
            // recycle the label
            label_pool_main.save_label(current_label);
            ...
            for (vertex_deg_t edge_id = 0; edge_id < Out_deg[current_vertex]; edge_id++)
            {
                ...
                // Build g-values of the extended path
                ...
                LABEL *new_label = label_pool_main.get_label();
                #ifdef PATH
...
////////////////////////////////////////////
// Binary search function using iterators
LABEL_TR_iter
binarySearch(LABEL_TR_list &Exp_labels_tr, cost_t key, size_t &comp_)
{
    LABEL_TR_iter begin = Exp_labels_tr.begin();
    LABEL_TR_iter end = Exp_labels_tr.end() - 1;
    while (begin <= end)
    {
        // comp_++;
        LABEL_TR_iter mid = begin + (end - begin) / 2;
        if ((*mid).f_[0] <= key) {
            begin = mid + 1; // Adjust the begin iterator
        } else {
            end = mid - 1; // Adjust the end iterator
        }
    }
    return end;
}
...
// but through binary search
std::pair<bool, LABEL_TR_iter>
dominance_check(LABEL_TR new_label_tr, LABEL_TR_list &Exp_labels_tr, size_t &comp_)
{
    bool dominated = false;
    // comp_++;
    if (Exp_labels_tr.empty() || new_label_tr.f_[0] < (Exp_labels_tr.front()).f_[0])
    {return std::make_pair(dominated, Exp_labels_tr.begin());}

    LABEL_TR_iter it = binarySearch(Exp_labels_tr, new_label_tr.f_[0], comp_);
    // comp_++;
    if ((*it).f_[1] <= new_label_tr.f_[1]) dominated = true;
    else if ((*it).f_[0] != new_label_tr.f_[0]) ++it;
